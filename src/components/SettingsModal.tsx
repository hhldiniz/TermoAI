...
export default function SettingsModal({
  ...
  const labelTitle = isPt ? 'Configurações' : 'Settings';
  const labelLang = isPt ? 'Idioma do Jogo' : 'Game Language';
  const labelSound = isPt ? 'Efeitos Sonoros' : 'Sound Effects';
  const labelHardMode = isPt ? 'Modo Difícil' : 'Hard Mode';
  const labelAutoClue = isPt ? 'Dica Automática' : 'Auto Reveal Hint';
  const labelAutoClueDesc = isPt ? 'Revela a pista da palavra após a 3ª tentativa.' : 'Reveals the word clue automatically after the 3rd guess.';
      ...
      >
        ...
        <div className="flex flex-col gap-4 py-1 select-none text-left">
          ...
          <div className="flex items-center justify-between border-b border-[#3a3a3c]/40 pb-3">
            <div className="flex flex-col gap-0.5 max-w-[70%]">
              ...
              <span className="text-[10px] text-[#818384]">
                {isPt ? 'Muda o dicionário e strings de UI.' : 'Toggles vocabulary & UI layout.'}
              </span>
            </div>
          ...
          <div className="flex items-center justify-between border-b border-[#3a3a3c]/40 pb-3">
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-[10px] text-[#818384]">
                {isPt ? 'Ativa sons sintetizados da retro-placa.' : 'Plays localized retro digital sound chimes.'}
              </span>
            </div>
            <button
              onClick={toggleSound}
          ...
          <div className="flex items-center justify-between pb-1">
            <div className="flex flex-col gap-0.5 max-w-[75%]">
              ...
              <span className="text-[10px] text-[#818384] leading-normal">
                {isPt ? 'Dicas reveladas precisam conter novas letras de palpites anteriores.' : 'Any revealed hints must be integrated in following attempts.'}
              </span>
            </div>
            <button
              onClick={() => {
        ...
        <div className="bg-[#121213] p-3 rounded border border-[#3a3a3c] text-[10px] text-slate-300 leading-normal text-left select-none">
          <p className="font-bold text-white uppercase tracking-widest mb-1 text-[9px] flex items-center gap-1.5">
            ...
            {isPt ? 'Como jogar Termo' : 'How to play Termo'}
          </p>
          ...
          <div className="flex flex-col gap-2 font-black uppercase tracking-wider text-[9px]">
            <div className="flex items-center gap-2">
              <span className="w-14 text-center bg-[#10b981] text-white rounded py-0.5">VERDE</span>
              <span className="text-[#818384] font-semibold tracking-tight normal-case">
                {isPt ? 'A letra faz parte da palavra e está na posição correta.' : 'The letter is present and in the exact index.'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-14 text-center bg-[#b59f3b] text-white rounded py-0.5">AMARELO</span>
              <span className="text-[#818384] font-semibold tracking-tight normal-case">
                {isPt ? 'A letra está contida na palavra, mas em uma posição incorreta.' : 'The letter is present but positioned elsewhere.'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-14 text-center bg-[#3a3a3c] text-[#818384] border border-[#3a3a3c] rounded py-0.5">CINZA</span>
              <span className="text-[#818384] font-semibold tracking-tight normal-case">
                {isPt ? 'A letra não faz parte de nenhuma seção da palavra secreta.' : 'The letter is absent from this secret word.'}
              </span>
            </div>
          </div>
        </div>
